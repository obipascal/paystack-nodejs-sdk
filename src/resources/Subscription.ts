...
export class Subscription {
    ...
    async enable(data: EnableSubscriptBody) {
        return await this.httpClient.post(
            HttpRequestEndpoints.subscription.enable,
            data
        );
    }
    ...
    async disable(data: EnableSubscriptBody) {
        return await this.httpClient.post(
            HttpRequestEndpoints.subscription.disable,
            data
        );
    }
    ...
    async generateUpdateSubLink(code: string) {
        return await this.httpClient.get(
            HttpRequestEndpoints.subscription.generateUpdateSubLink.replace(
                ":code",
                code
            )
        );
    ...
    async sendUpdateSubLink(code: string) {
        return await this.httpClient.post(
            HttpRequestEndpoints.subscription.sendUpdateSubLink.replace(
                ":code",
                code
            )
        );
